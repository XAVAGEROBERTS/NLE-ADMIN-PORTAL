// LecturerAssignmentsManager.jsx - COMPLETE WITH FILE VIEWING FOR LECTURERS
...
const LecturerAssignmentsManager = ({ profile, courses, stats, showToast, mode = "create" }) => {
  ...
  // ===== ASSIGNMENT FILES VIEW STATE =====
  const [showAssignmentFiles, setShowAssignmentFiles] = useState(false);
  const [assignmentFileUrls, setAssignmentFileUrls] = useState([]);
  const [downloadingAssignmentFile, setDownloadingAssignmentFile] = useState(null);
  ...
  const handleViewAssignmentFiles = (assignment) => {
    console.log("📂 Viewing assignment files:", assignment.id, assignment.title);
    
    // Generate download URLs for the assignment files
    const projectRef = supabase.supabaseUrl.split("//")[1].split(".")[0];
    
    const fileUrls = (assignment.file_urls || [])
      .map((filePath) => {
        ...
        const cleanPath = filePath.startsWith("/") ? filePath.slice(1) : filePath;
        const publicUrl = `https://${projectRef}.supabase.co/storage/v1/object/public/assignments/${cleanPath}`;
    ...
    setAssignmentFileUrls(fileUrls);
    setShowAssignmentFiles(true);
  };

  const closeAssignmentFilesView = () => {
    setShowAssignmentFiles(false);
    setAssignmentFileUrls([]);
  };

  const downloadAssignmentFile = async (fileUrl, fileName) => {
    try {
      setDownloadingAssignmentFile(fileName);

      console.log("📥 Downloading assignment file:", { fileUrl, fileName });

      if (fileUrl.startsWith("http")) {
        window.open(fileUrl, "_blank");
        setDownloadingAssignmentFile(null);
        return;
      }
  ...
  // ===== ASSIGNMENT FILES VIEW MODAL =====
  if (showAssignmentFiles) {
    return (
        ...
        <div className="lecturer-tab-header">
          <div>
            ...
            <p style={{ fontSize: "14px", color: "#1976d2", fontWeight: "500" }}>
              {selectedAssignment?.title || "Assignment"} - Files uploaded by lecturer
            </p>
          </div>
          <button 
            className="lecturer-cancel-btn" 
            onClick={closeAssignmentFilesView}
          >
            ← Back to Assignments
          </button>
        </div>

        {assignmentFileUrls.length === 0 ? (
          ...
          <div style={{ 
            ...
            <h3 style={{ marginBottom: "16px" }}>📎 Attached Files ({assignmentFileUrls.length})</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {assignmentFileUrls.map((file, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                      ...
                      <button
                        className="lecturer-course-btn"
                        onClick={() => downloadAssignmentFile(file.url, fileName)}
                        disabled={downloadingAssignmentFile === fileName}
                        style={{ 
                          background: "#1976d2", 
                          color: "white",
                          fontSize: "12px",
                      ...
                      >
                        {downloadingAssignmentFile === fileName ? "⏳" : "📥 Download"}
                      </button>
                    </div>
                  </div>
                );
        ...
        <div style={{ 
          ...
          {selectedAssignment.file_urls && selectedAssignment.file_urls.length > 0 && (
            <button
              className="lecturer-course-btn"
              onClick={() => {
                // Close submissions and show assignment files
                setSelectedAssignment(null);
                setAssignmentSubmissions([]);
                setSelectedSubmissions([]);
                setGradeForm({});
                handleViewAssignmentFiles(selectedAssignment);
              }}
              style={{ 
                background: "#fd7e14", 
                color: "white",
            ...
            >
              📂 View Assignment Files ({selectedAssignment.file_urls.length})
            </button>
          )}
        </div>
                  ...
                  <td>
                    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                      ...
                      {assignment.file_urls && assignment.file_urls.length > 0 && (
                        <button 
                          className="lecturer-course-btn"
                          onClick={() => handleViewAssignmentFiles(assignment)}
                          style={{ 
                            background: "#fd7e14", 
                            color: "white", 
                            fontSize: "12px", 
                        ...
                        >
                          📂 Attached CW ({assignment.file_urls.length})
                        </button>
                      )}
