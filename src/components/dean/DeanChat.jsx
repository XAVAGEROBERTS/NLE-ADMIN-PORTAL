// dean/DeanChat.jsx - REAL-TIME WITH AUTO-SCROLL (MIRRORING HOD)
...
const DeanChat = ({
  ...
  const scrollToBottom = useCallback((behavior = 'smooth') => {
    ...
    setTimeout(() => {
      ...
      const chatBody = document.querySelector('.dean-chat-body');
      if (chatBody) {
  ...
  useEffect(() => {
    if (!selectedUser?.email || !deanEmail) {
      console.log('⚠️ No selected user or Dean email, skipping realtime setup');
      return;
    }

    console.log('🔄 Setting up real-time chat for Dean:', deanEmail, '↔', selectedUser.email);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (msg.sender_email === selectedUser.email && msg.receiver_email === deanEmail) {
            supabase
              .from('chat_messages')
              .update({ is_read: true })
              .eq('id', msg.id)
    ...
    const pollInterval = setInterval(() => {
      if (!selectedUser?.email || !deanEmail) return;

      supabase
        .from('chat_messages')
        .select('*')
        .or(
          `and(sender_email.eq.${deanEmail},receiver_email.eq.${selectedUser.email}),` +
          `and(sender_email.eq.${selectedUser.email},receiver_email.eq.${deanEmail})`
        )
  ...
  }, [selectedUser?.email, deanEmail, scrollToBottom]);
  ...
  }, [localNewMessage, selectedUser, deanEmail, deanName, selectedUserType, localSending, scrollToBottom, profile]);
        ...
        >
          {localMessages.length === 0 ? (
            ...
            localMessages.map((msg) => {
              const isMe = msg.sender_role === 'dean' || msg.sender_email === deanEmail;
              const isTemp = typeof msg.id === 'string' && msg.id.startsWith('temp-');

              return (
                <div
                  key={msg.id}
                  className={`dean-chat-message ${isMe ? 'sent' : 'received'}`}
                  style={{
